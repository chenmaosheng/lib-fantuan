...
public:
	...
	bool	Push(int32 iLogLevel, wchar_t* strFormat, ...);
	int32	GetLogLevel() const
	{
		return m_iLogLevel;
	}	

	wchar_t*	_Level2String(int32 iLogLevel);
	static uint32 __stdcall _LogOutput(void*);
	void _Tick();
...
#define LOG_DBG(Expression, ...)										\
	...
	EasyLog::Instance()->Push(EasyLog::LOG_DEBUG_LEVEL, _T("[%u:%-30s][LINE:%-4u] ") Expression, GetCurrentThreadId(), _T(__FUNCTION__), __LINE__, ##__VA_ARGS__);

#define LOG_WAR(Expression, ...)										\
	if (EasyLog::Instance()->GetLogLevel() <= EasyLog::LOG_WARNING_LEVEL)				\
	EasyLog::Instance()->Push(EasyLog::LOG_WARNING_LEVEL, _T("[%u:%-30s][LINE:%-4u] ") Expression, GetCurrentThreadId(), _T(__FUNCTION__), __LINE__, ##__VA_ARGS__);

#define LOG_ERR(Expression, ...)										\
	if (EasyLog::Instance()->GetLogLevel() <= EasyLog::LOG_ERROR_LEVEL)					\
	EasyLog::Instance()->Push(EasyLog::LOG_ERROR_LEVEL, _T("[%u:%-30s][LINE:%-4u] ") Expression, GetCurrentThreadId(), _T(__FUNCTION__), __LINE__, ##__VA_ARGS__);

#define LOG_STT(Expression, ...)										\
	if (EasyLog::Instance()->GetLogLevel() <= EasyLog::LOG_STATE_LEVEL)				\
	EasyLog::Instance()->Push(EasyLog::LOG_STATE_LEVEL, _T("[%u:%-30s][LINE:%-4u] ") Expression, GetCurrentThreadId(), _T(__FUNCTION__), __LINE__, ##__VA_ARGS__);
