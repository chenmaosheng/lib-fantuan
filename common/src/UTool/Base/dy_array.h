...
public:
									...
									m_iCount(0)
	{
		...
		if (TypeTraits<T>::isScalar)
		{
			...
			{
				std::copy(rhs.m_Elements, rhs.m_Elements + size, m_Elements);	
			}
			else
			{
				std::copy(rhs.m_pDynamicElements, rhs.m_pDynamicElements + size, m_pDynamicElements);
			}
		}
	...
	{
		iterator _where = (iterator)value;
		if (TypeTraits<T>::isScalar)
		{
